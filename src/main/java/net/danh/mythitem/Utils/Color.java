...
public class Color {
    ...
    static {
        try {
            ...
            CHAT_COLOR_FROM_COLOR = ChatColor.class.getDeclaredMethod("of", java.awt.Color.class);
    ...
    public static String colorize(String text, char colorSymbol) {
        ...
        while (g.find()) {
            java.awt.Color start = java.awt.Color.decode(g.group(1));
            String between = g.group(2);
            java.awt.Color end = java.awt.Color.decode(g.group(3));
        ...
        while (r.find()) {
            if (hexSupport) {
                ChatColor color = fromColor(java.awt.Color.decode(r.group(1)));
    ...
    private static String rgbGradient(String text, java.awt.Color start, java.awt.Color end, char colorSymbol) {
        ...
        for (int i = 0; i < characters.size(); i++) {
            ...
            ChatColor current = fromColor(new java.awt.Color((int) Math.round(red[i]), (int) Math.round(green[i]), (int) Math.round(blue[i])));
    ...
    private static java.awt.Color fromChatColor(ChatColor color) {
        try {
            return (java.awt.Color) COLOR_FROM_CHAT_COLOR.invoke(color);
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new RuntimeException(e);
        }
    }

    private static ChatColor fromColor(java.awt.Color color) {
        try {
